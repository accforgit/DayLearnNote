...
## 在 `MarkDown`中写数学公式
`MarkDown`支持 `LaTex`语法，但是这语法可能有点复杂，懒得学，然而又想在 `MarkDown`中写数学公式，此 [网站](https://webdemo.myscript.com/views/math.html)提供在浏览器中直接手写数学公式，然后将其转化为包括 `LaTex`、`MathML`、`SymbolTree`在内的语法，然后在 `MarkDown`文件中引入下面这段 `js`（有的在线 `md`编辑器可能已经帮你引好了），接着直接将转化好的公式复制到 `md`文件中即可~
