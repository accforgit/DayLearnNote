...
[转载自 https://github.com/markyun/My-blog/tree/master/Front-end-Developer-Questions/Questions-and-Answers](https://github.com/markyun/My-blog/tree/master/Front-end-Developer-Questions/Questions-and-Answers)
...
```
此属性存在四种属性值，默认为visible，表示可见，hidden表示元素不可见，当为collapse的时候，当在表格元素中使用时，此值可删除一行或一列，但是它不会影响表格的布局，被删除的一行或一列将会呈现为空白。如果此值被用在其他的元素上，会呈现为 "hidden"。
```
...
```
元素横向和竖向百分比设定都是相对于父元素
```
- 全屏滚动的原理是什么？用到了CSS的那些属性？

- 什么是响应式设计？响应式设计的基本原理是什么？如何兼容低版本的IE？
```
指网页能自动识别屏幕宽度、并做出相应调整的网页设计。主要是针对现在互联网用户使用多种终端浏览同一个网页的行为趋势，如果每个网站为此就得制作一个App，成本高且安装率低，完全没有必要，所以自适应网页设计其实才是未来的趋势，尤其小网站。
```
...
webp:谷歌（google）开发的一种旨在加快图片加载速度的图片格式。图片压缩体积大约只有JPEG的2/3，并能节省大量的服务器带宽资源和数据空间。
```
...
```
.bind()是直接绑定在元素上
.live()则是通过冒泡的方式来绑定到元素上的。更适合列表类型的，绑定到document DOM节点上。和.bind()的相比优势是支持动态数据，也就是说可以绑定当前或未来的元素（比如由脚本创建的新元素）
.delegate()则是更精确的小范围使用事件代理，性能优于.live()，同样支持动态数据。
是最新的1.9版本整合了之前的三种方式的新事件绑定机制，推荐使用。
```
...
```
1. [JS自定义事件](http://www.jianshu.com/p/418e9e35d5a1)
2. callbacks.fire( arguments )用于传入指定的参数调用所有的回调:
	$(function () { 
	    // 将被添加到列表的一个简单的函数
...
-  哪些操作会造成内存泄漏？
	...
	- 意外的全局变量
		...
		```js
		function foo() {
     	this.bar2 = '默认绑定this指向全局' // 全局变量=> window.bar2，不会自动垃圾回收
  		bar = '全局变量'; // 没有声明变量 实际上是全局变量=>window.bar，不会自动垃圾回收
    }
    foo();
		```
		解决方法：在函数内使用严格模式 `or` 手动释放不再适用的全局变量
		```js
			// 严格模式
			"use strict"; 
			// 手动释放的两种方式
	...
	- 闭包
		...
		```
		解决方法：手动解除引用，例如 `obj = null`
	- 控制台日志，例如 `console`保存大量数据在内存中。
		过多的`console`，比如定时器的`console`会导致浏览器卡死。
		解决：合理利用 `console`，线上项目尽量少的使用 `console`。
